...
function Bisection (){
    ...
    const setExample = () =>{
      api.getMovieByName("One-Point-Iteration-Method").then(db => {
    ...
    const onepointCal = ([fx,x,er]) => {
      ...
      const onepointCalculate = (x,er) => { 
        ...
        return Math.abs(j)<er ? xnew : onepointCalculate(xnew,er); 
    }
      ...
      if (er === '' || x === '' || fx === ''){
        ...
        const res = onepointCalculate(x,er);
    ...
    return (
        <Layout>
          <Content style={{ margin: '10px 10px 0' }}>
            <div style={{ padding: 20, background: '#ACDBF3', textAlign: 'center' }}>
            <Row gutter={[8, 40]}>
              <Col className="text-left"  span={24}>
               <h4 >One-Point Iteration Method Algorithm</h4>
               <br/>
               <h6>ใส่ fx x er</h6>
              </Col>
            </Row>
            ...
            <Col span={6} />
              <Col span={6} >
                <Button type="primary" shape="round" icon="stock" size="Large"
                 onClick={()=>onepointCal([fx,parseFloat(x),parseFloat(er)])}>
                  Calculate
                </Button>
              </Col>
              <Col span={6} >
